...
The notebook should be placed inside the order management folder, so that it has the same root directory as the **data** folder.
...
The model builds conditional distributions of future price movements using only past information upto the latest full interval to avoid look-ahead bias. State classification, EPDF construction and order processing are carried out in parallel, on rolling basis. 
...
- **Interval slicing**
  ...
  - `Volatility`: Standard deviation of the the log return for each interval (before annualisation)
...
- **VWAP-EPDF extension**
  In addition to the three core strategies, we implement a VWAP-EPDF extension. Agent orders are first aggregated into parent orders and distributed across future execution intervals according to a rolling expected intraday volume profile estimated from historical trading days only. This generates a sequence of child orders that follow the expected market liquidity pattern.The resulting child orders are then executed using the same EPDF repricing and resubmission framework described above. This extension allows us to evaluate whether volume-aware order scheduling can further improve execution quality relative to direct EPDF execution.  
  
- **Evaluation Framework**

  Four strategies are compared to isolate the effect of repricing and resubission:
...
| **4. VWAP-EPDF extension** | First distribute parent orders according to a rolling expected intraday volume profile, then apply the same EPDF repricing and resubmission framework. |
...
The parameter with the highest validation score is selected
...
| `apply_rolling_expected_vwap_forward` | `df_agent_order`=pd.DataFrame,<br>`merged`=pd.DataFrame,<br>`df_state_all`=pd.DataFrame,<br>`participation_rate`=float,<br>`alpha_toxicity`=float | Constructs a rolling expected intraday volume profile using historical trading days only, adjusts participation rates according to EPDF toxicity states, and allocates agent orders into VWAP child orders subject to participation constraints. | `df_vwap_orders`=pd.DataFrame,<br>`df_unfilled`=pd.DataFrame |
...
| `df_vwap_orders` | `pd.DataFrame` | VWAP-generated child orders obtained from the rolling expected volume profile. These orders are subsequently executed using the same EPDF repricing and resubmission framework. |
...
- `trade_cost` = `commission` * abs(`action`)
- `position` += `action`
- `cash` += `trade_cashflow` - `trade_cost`
...
-**`rolling_backtest_with_optimal_params`**
  ...
  - Logic
    ...
    In addition to the three core strategies, the function also evaluates the VWAP-EPDF extension. It first generates VWAP child orders using `apply_rolling_expected_vwap_forward`, and then executes these child orders using the same EPDF repricing and resubmission logic.
